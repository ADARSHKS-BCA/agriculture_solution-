...
public class ScanController : Controller
{
    ...
    public async Task<IActionResult> Analyze(CropAnalysisViewModel model)
    {
        ...
        try
        {
            ...
            var (disease, confidence, riskLevel, riskExplanation, fullResult, success, error) = await _diseaseService.PredictAsync(
                model.ImageFile!, 
                model.CropType ?? "Unknown", 
                model.City, 
                model.Latitude, 
                model.Longitude
            );
            ...
            if (success && fullResult != null)
            {
                // Build result with real AI prediction
                result = BuildResult(model, imagePath, disease, confidence, riskLevel, riskExplanation, fullResult);
            }
            else
            {
    ...
    /// </summary>
    private AnalysisResultViewModel BuildResult(CropAnalysisViewModel input, string imagePath, string disease, float confidence, string riskLevel, string riskExplanation, DiseaseDetectionService.FastApiResponse backendResult)
    {
        var region = string.IsNullOrWhiteSpace(input.City)
            ? "Unknown Location"
            : input.City;
        ...
        if (backendResult.Weather_Analysis != null)
        {
            temp = backendResult.Weather_Analysis.Temperature ?? temp;
            hum = backendResult.Weather_Analysis.Humidity ?? hum;
            rain = backendResult.Weather_Analysis.Rainfall ?? rain;
            wind = backendResult.Weather_Analysis.Wind_Speed ?? wind;
        }
